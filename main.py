...
class Wine:
    ...
    def get_alcohol(self):
        return f"{self.alcohol // 10}-{self.alcohol // 10 + 0.9}%"
    ...
    def get_earth_inorganic(self):
        if self.earth_inorganic >= 230:
            ...
            return "Wet Stone"
        elif self.get_earth_inorganic >= 0:
            return "None"
        else:
...
class TastingNote:
    ...
    def generate_description(self):
        output = []
        output.append(wine.get_clarity())
        output.append(wine.get_appearance())
...
try:
    ...
    conn.row_factory = sqlite3.Row      # necessary to set this so row_factory will work as desired
    c = conn.cursor()
    c.execute('SELECT * FROM wines')
    rows = c.fetchall()

    # now let's convert each of these into an object
    for row in rows:
        row_dict = dict(row)
        wine = Wine(**row_dict)
        output.append(wine)
